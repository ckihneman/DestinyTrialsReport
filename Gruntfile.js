        ...
        heroku: {
            files: [{
                ...
                method: function(fs, fd, done) {
                    ...
                    fs.writeSync(fd, '  res.setHeader("Access-Control-Allow-Origin", "*");\n');
                    fs.writeSync(fd, '  var api_key = process.env.BUNGIE_API;\n');
                    fs.writeSync(fd, '  res.setHeader("Access-Control-Allow-Headers", "X-Requested-With");\n');
                    fs.writeSync(fd, '  res.setHeader("X-API-Key", api_key);\n');
                    fs.writeSync(fd, '  try {request(("http://www.bungie.net/Platform/" + req.originalUrl.replace("/bungie/", "")), {encoding: null}, function(error, response, body) {\n');
